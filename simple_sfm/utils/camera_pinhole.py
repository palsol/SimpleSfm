...
class CameraPinhole:
    ...
    def to(self, device: str):
        """
        Move cameras to device.

        Args:
            device: device
        """
        if getattr(self, 'extrinsics', None) is not None:
            self.extrinsics = self.extrinsics.to(device=device)
        ...
        if getattr(self, 'images_sizes', None) is not None:
            self.images_sizes = self.images_sizes.to(device=device)
    ...
    def world_view_direction(self, axis: str = 'z') -> torch.Tensor:
        ...
        cam_direction = torch.tensor(direction,
                                     ...
                                     ).expand_as(world_position).unsqueeze(-2)
